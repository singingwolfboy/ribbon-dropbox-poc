...
--! Hash: sha1:ef024d5efcf5c2e16ef03092f175885995390b1d
...
grant
  select,
  insert (name),
  update (name, dropbox_preapproval_file_request_id, has_preapproval),
  delete
...
begin
  perform graphile_worker.add_job(tg_argv[0], json_build_object('id', OLD.id, 'user_id', OLD.user_id, 'name', OLD.name));
  return OLD;
end;
...
DROP TRIGGER if exists _200_add_dropbox ON app_public.clients;
CREATE TRIGGER _200_add_dropbox AFTER INSERT ON app_public.clients FOR EACH ROW EXECUTE FUNCTION app_private.tg__add_job('client__add_to_dropbox');
...
DROP TRIGGER if exists _200_remove_dropbox ON app_public.clients;
CREATE TRIGGER _200_remove_dropbox BEFORE DELETE ON app_public.clients FOR EACH ROW EXECUTE FUNCTION app_private.tg__before_client_delete__add_job('client__remove_from_dropbox');
